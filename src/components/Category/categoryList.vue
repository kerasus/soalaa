<template>
  <div class="category-body">
    <q-card>
      ...
      <q-list>
        <q-item v-for="(item, index) in filteredItems"
                :key="index">
          ...
          <q-item-section>
            ...
            <q-input v-else-if="item.editable && !item.editMode"
                     v-model="item.title"
                     class="category-input"
                     color="grey-3"
                     outlined
          ...
          <q-item-section side>
            <div class="text-grey-8 q-gutter-xs">
              <div v-if="!item.editable && !item.editMode">
                <q-btn class="gt-xs text-light-blue"
                       flat
                       dense
                       round
              ...
              <div v-else-if="item.editable && !item.editMode">
                <q-btn class="gt-xs text-green"
                       flat
                       dense
                       round
                       icon="mdi-check-circle-outline"
                       @click="create(item)" />
                <q-btn class="gt-xs text-red"
                       flat
                       dense
                       round
