<template>
    <v-card>
            ...
            <v-container>
                <v-row>
                    <v-col cols="12">
                        ...
                        <div v-if="canEnterCode" class="d-flex timer-box">
                               <div>
                                   <p class="text-size"> زمان باقی مانده </p>
                                   <div class="d-flex">
                                   <span
        ...
        <v-card-actions class="d-flex flex-column">
            ...
            ></v-progress-circular>
            <v-btn
                v-if="!canEnterCode && !loading && canSendCodeAgain"
                block
                dark
                color='red'
                class="mb-3"
    ...
    methods: {
        ...
        checkApiUrl() {
            if (this.phoneNumber.source === 'login') {
                ...
                this.verifyApi = 'verify'
                return
            }
            this.resendApi = 'resendToGuest'
            this.verifyApi = 'verifyGuest'
        },
        sendVerifyCodeToUser() {
            this.loading = true
            axios.get('/api/v2/mobile/' + this.resendApi, {params: {'mobile': this.phoneNumber.phone,}})
                .then(response => {
        ...
        checkVerifyCode() {
            this.loading = true
            axios.post('/api/v2/mobile/' + this.verifyApi, {mobile: this.phoneNumber.phone, code: this.verifyCode})
                .then(response => {
