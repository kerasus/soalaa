...
md.use(require('markdown-it-container'), 'beit', {
  ...
  render: function (tokens, idx) {
    const m = tokens[idx].info.trim().match(/^beit\s+(.*)--\*mesra\*--(.*)$/)
    if (m && m[1] && m[2] && tokens[idx].nesting === 1) {
      const mesra1 = md.render(md.utils.escapeHtml(m[1].toString()))
      const mesra2 = md.render(md.utils.escapeHtml(m[2].toString()))
      // opening tag
